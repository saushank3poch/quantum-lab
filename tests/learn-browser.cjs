...
(async () => {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const reading=await page.locator('#learn-reading').textContent();
    if(i<2)assert.ok(reading.includes('read '+result.outcomes[0]));
    else result.counts.forEach((count,j)=>{
      if(result.expected[j]>1e-12)assert.ok(reading.includes(count+' readings of '+j.toString(2).padStart(i>5?2:1,'0')));
    });
    assert.equal(await page.locator('#learn-explanation li').count(),i<2?2:3);
    assert.ok(await page.locator('#learn-meaning').isVisible());
    assert.ok(await page.locator('#learn-next').isDisabled());
    ...
    assert.match(await page.locator('#learn-check-feedback').textContent(),/^Yes\./);
    if (i === 4) {
  ...
  assert.match(await page.locator('#learn-reading').textContent(),/40 readings of 00/);
